...
{
    public partial class FrmAKADEMISYENGUNCELLE : Form
    {
        public FrmAKADEMISYENGUNCELLE()
        {
            InitializeComponent();
        }
                    ...
                    if (eskiSifre == Txt_EskiSifre.Text)
                    {
                        if (TXT_Sifre_Degistirme_Alani1.Text == TXT_Sifre_Degistirme_Alani2.Text)
                        {
                            if ((TXT_Sifre_Degistirme_Alani1.Text != eskiSifre) && (TXT_Sifre_Degistirme_Alani2.Text != eskiSifre))
                            {
                                BTN_Guncelle.Enabled = true;
                                baglanti.Open();
                                SqlCommand komut2 = new SqlCommand("UPDATE Tbl_Egitimci SET EGISIFRE=@P1 WHERE EGINO=@P2", baglanti);
        ...
        private void FrmAKADEMISYENGUNCELLE_Paint(object sender, PaintEventArgs e)
        {
            ControlPaint.DrawBorder(e.Graphics, this.ClientRectangle, Color.Black, ButtonBorderStyle.Solid);
        }

        private void FrmAKADEMISYENGUNCELLE_Load(object sender, EventArgs e)
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("select EGISIFRE from Tbl_Egitimci where EGINO=@P1", baglanti);
            komut.Parameters.AddWithValue("@p1", EGINO);
            SqlDataReader oku = komut.ExecuteReader();

            while (oku.Read())
            {
                eskiSifre = oku["EGISIFRE"].ToString();
